...
jQuery(document).ready(function()
{
	...
	jQuery(document.body).on('keyup', '.lp-location-search-field-openweathermaps', _.debounce( function()
	{
		if( jQuery(this).val() != "")
		{
			...
			jQuery('#lp-owm-spinner-' + location_id).removeClass("hidden");
		
			// PING
			var data = { action: 'lp_ping_owm_for_id', location: jQuery(this).val(), units: units_val };
			jQuery.getJSON(ajaxurl, data, function(response) 
			{
				...
				if( !places )
				{
					jQuery('#owmid-selector-' + location_id).html( "<span style='color:red;'>" + lp_script.no_owm_city + "</span>");
				}
				else
				{
					if(place_count == 1)
					{
						...
						jQuery( '#owmid-selector-' + location_id ).html( "<span style='color:red;'>" + lp_script.one_city_found + "</span>" );
					}
					else
					{
						var rtn = lp_script.confirm_city;
				...
				jQuery('#lp-owm-spinner-' + location_id).addClass("hidden");
			});
		}	
	}, 250));
});